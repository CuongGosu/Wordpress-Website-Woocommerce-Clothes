...
function changeprice() {
  $.get('/cart.js').done(function (dataTotal) {
    let dataid = '';
    let arr = [];
    $.each(dataTotal.items, function (index, data) {
      dataid = data.variant_id;
      arr.push(dataid);
    });
    var el = '';
    $.ajax({
      ...
      success: function (result) {
        ...
        const output = obj.items.map((item) => {
          if (item.compare_at_price > item.price && item.compare_at_price > 0) {
            $(`.pro-price-view .price-original.${item.id} del`).html(
              Haravan.formatMoney(item.compare_at_price, formatMoney)
            );
          }
        });
      },
    ...
    if (cart) {
      ...
      jQuery.each(cart.items, function (i, item) {
        ...
        tr = jQuery('.original')
          ...
          .appendTo('table#cart-table tbody');
        if (item.image != null)
          tr.find('.item-image').html(
            '<img src=' + Haravan.resizeImage(item.image, 'small') + '>'
          );
        else
          tr.find('.item-image').html(
            "<img src='//theme.hstatic.net/200000540675/1000904234/14/no_image.jpg?v=364'>"
          );
        vt = item.variant_options;
        if (vt.indexOf('Default Title') != -1) vt = '';
        tr.find('.item-title')
          .children('a')
          .html(item.product_title + '<br><span>' + vt + '</span>')
          .attr('href', item.url);
        tr.find('.item-quantity').html(
          "<input id='quantity1' name='updates[]' min='1' type='number' value=" +
            item.quantity +
            " class='' />"
        );
        if (typeof formatMoney != 'undefined') {
          tr.find('.item-price').html(
            Haravan.formatMoney(total_line, formatMoney)
          );
        } else {
          tr.find('.item-price').html(Haravan.formatMoney(total_line, ''));
        }
        tr.find('.item-delete').html(
          "<a href='javascript:void(0);' onclick='deleteCart(" +
            (i + 1) +
            ")' ><i class='fa fa-times'></i></a>"
        );
      });
      jQuery('.item-total').html(
      ...
      if (cart.item_count == 0) {
        ...
        jQuery('#cart-view').html(
          '<tr class="item-cart_empty"><td><div class="svgico-mini-cart"> <svg width="81" height="70" viewBox="0 0 81 70"><g transform="translate(0 2)" stroke-width="4" stroke="#1e2d7d" fill="none" fill-rule="evenodd"><circle stroke-linecap="square" cx="34" cy="60" r="6"></circle><circle stroke-linecap="square" cx="67" cy="60" r="6"></circle><path d="M22.9360352 15h54.8070373l-4.3391876 30H30.3387146L19.6676025 0H.99560547"></path></g></svg></div> Hiện chưa có sản phẩm</td></tr>'
        );
        jQuery('#cartform').hide();
      } else {
        jQuery('#exampleModalLabel').html(
          'Bạn có ' + cart.item_count + ' sản phẩm trong giỏ hàng.'
        );
...
function clone_item(product, i) {
  ...
  if (product.image == null) {
    item_product
      ...
      .attr(
        'src',
        '//theme.hstatic.net/200000540675/1000904234/14/no_image.jpg?v=364'
      )
  ...
  item_product
    ...
    .html(
      '<span class="price-original ' +
        ...
        Haravan.formatMoney(product.price, formatMoney) +
        '</span>'
    );
  item_product
    .find('.remove-cart')
...
$(document).on('lazyloaded', function (e) {
  ...
  fixHeightProduct(
    '#collection-body .content-product-list',
    '.product-resize',
    '.image-resize'
  );
  jQuery(window).resize(function () {
...
function boxAccount(type) {
  ...
  $('.site_account .site_account_panel_list .site_account_panel ').addClass(
    'd-none'
  );
  $('.site_account .site_account_panel_list .site_account_panel ').removeClass(
    'is-selected'
  );
  $(
...
jQuery(document).ready(function () {
  ...
  jQuery(window).scroll(function () {
    ...
    if (
      ...
      if (
        jQuery(window).scrollTop() > offset_sticky_header + 150 &&
        jQuery(window).scrollTop() - 150 + jQuery(window).height() <
          $(document).height()
      ) {
...
$('.ultimate-search').submit(function (e) {
  ...
  if (q.indexOf('script') > -1 || q.indexOf('>') > -1) {
    alert(
      'Từ khóa của bạn có chứa mã độc hại ! Vui lòng nhập lại key word khác'
    );
